...
import { calcCostWithTaxes, totalAppliancesConsumption, calculateMarketExpenses, calculateVehicleExpenses } from '../finance/budget';
...
export function calculateBudgetComparison(input: BudgetInput): BudgetResult {
  ...
  const vehicles = input.vehicles ? calculateVehicleExpenses(input.vehicles) : undefined;
